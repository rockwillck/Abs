...
function renderAb(points) {
    myPoints = []
    for (point of points) {
        myPoints.push(point.x)
        myPoints.push(point.y)
    }
...
function draw() {
    ...
    for (abIndex=0;abIndex<xPack;abIndex+=2) {
        ...
        period3 = periods[abIndex + 2]
        if (abIndex == xPack - 2) {
            period3 = canvas.height
        }
        ctx.fillStyle = "wheat"
        renderAb([
            {x:0, y:period1},
        ...
        renderAb([
            {x:canvas.width/2 + 50, y:period1},
            {x:canvas.width - line, y:period2},
            {x:canvas.width, y:period1},
            {x:canvas.width, y:period2},
            {x:canvas.width - line, y:period3},
            {x:canvas.width/2 + 50, y:period2},
            {x:canvas.width/2 + 50, y:period1}
        ]
        )
